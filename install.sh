...
function _cmd {
    # empty zeronehardening.log
    > zeronehardening.log
    # hide stdout, on error we print and exit
    if eval "$1" 1> /dev/null 2> zeronehardening.log; then
        return 0 # success
    fi
    ...
    while read line; do 
        printf "      ${line}\n"
    done < zeronehardening.log
    printf "\n"
    # remove log file
    rm zeronehardening.log
    # exit installation
    exit 1
} 
...
\_____/\___|_|  \___/|_| |_|\___|
 ...
 ${LBLACK}Hardening ${YELLOW}Ubuntu 20.04 ${LBLACK}https://zeronemicrosystems.com
...
_task "update sysctl.conf"
    _cmd 'wget --timeout=5 --tries=2 --quiet -c https://raw.githubusercontent.com/Zerone-MS/ubuntuhardening/main/sysctl.conf -O /etc/sysctl.conf'

# description
_task "update sshd_config"
    _cmd 'wget --timeout=5 --tries=2 --quiet -c https://raw.githubusercontent.com/Zerone-MS/ubuntuhardening/main/sshd.conf -O /etc/ssh/sshd_config'
